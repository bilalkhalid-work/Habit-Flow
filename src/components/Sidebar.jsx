...
function Sidebar({ mobileOpen, setMobileOpen }) {
  ...
  return (
    <>
      ...
      <div className={`fixed left-0 top-0 h-full w-64 z-20 backdrop-blur-xl border-r ${theme.sidebar} border-white/10 flex flex-col transition-transform duration-300
        ...
        <div className="p-6 border-b border-white/10">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => setMobileOpen(false)}
              className={`md:hidden ${theme.textMuted} hover:${theme.text}`}
            >
              ✕
            </button>
          </div>
          <p className={`text-xs ${theme.textMuted} mt-3 italic leading-relaxed`}>{theme.tagline}</p>
        </div>
